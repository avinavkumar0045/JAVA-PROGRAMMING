...
public class sevenReverseADLL {
    ...
    public void ReverseDLL(){
        ...
        while(curr != null){
            next = curr.next;
            curr.next  = prev;
            curr.prev = next ; // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&( 3 var 5 steps)
            prev = curr;
            curr = next;

        }    
        head = prev;    // IMPORTANT LINE   
        
    }
    public static void main(String[] args) {

        sevenReverseADLL dll = new sevenReverseADLL();
        ...
        dll.print();
        dll.ReverseDLL();
        System.out.println("After revesing the DLL");
        dll.print();
