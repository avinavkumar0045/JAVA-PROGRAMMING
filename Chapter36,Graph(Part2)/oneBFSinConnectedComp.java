import java.util.*;
public class oneBFSinConnectedComp {
    static class Edge{
        int src;
        int  dest;
        int wt;
    ...
    public static void BFSUtil( ArrayList<Edge> graph[] , boolean vis[] ){ // O( V + E)
      ...
      public static void DFS( ArrayList<Edge> graph[] ) {
        ...
        for( int i =0; i<graph.length;i++){
            ...
            if( !vis[i]){ // if vis[i] is false 
            DFSUtil(graph,i, vis); // curr = i
            }
        }        
    }
    // DFSUtil  O(V + E)
    public static void DFSUtil(ArrayList<Edge> graph[] , int curr , boolean vis[]){ // for int curr , just pass the vertex number 
        //visit
        System.out.print(curr +" ");
